...
bool verificar_rango(vector<vector<char>>& sopa, int i, int j) {
    ...
    for(int i = 0; i < sopa_letras.size(); i++){
        for(int j = 0; j < sopa_letras[i].size(); j++){
            if(sopa_letras[i][j] == 'X'){
                ...
                // Comprobamos hacia la derecha
                if (verificar_rango(sopa_letras, i, j + 3) &&
                    sopa_letras[i][j + 1] == 'M' &&
                    sopa_letras[i][j + 2] == 'A' &&
                    sopa_letras[i][j + 3] == 'S') {
                    total++;
                ...
                if (verificar_rango(sopa_letras, i + 3, j) &&
                    sopa_letras[i + 1][j] == 'M' &&
                    sopa_letras[i + 2][j] == 'A' &&
                    sopa_letras[i + 3][j] == 'S') {
                    total++;
                ...
                if (verificar_rango(sopa_letras, i - 3, j + 3) &&
